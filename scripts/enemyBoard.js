    ...
    update : function() {
        ...
        for(var y=0; y <= 21; y++) {// every row
            for(var x=0; x<= 9; x++) { //go through every block in the row
                ...
                //if not empty
                if(positions[i]>0){
                    draw.draw(1 + x * 24, 1 + (y - enemyBoard.enemyGarbage) * 24, 0, 0, color, enemyBoard.canvas);
                }
                i++;
            }
        }
        ...
        if(check){
            ...
            enemyBoard.playerKo++;
            tetris.sendLines(0,'not'); // update record
        }
        
        //draw garbage
        for(var y=21; y > 21 - enemyBoard.enemyGarbage; y--) {// every row
            for(var x=0; x<= 9; x++) { //go through every block in the row
                draw.draw(1 + x * 24, 1 + (y) * 24, 0, 0, "#d3d3d3", enemyBoard.canvas);
            }
        }
